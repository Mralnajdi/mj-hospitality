(async()=>{
  ...
  function home(){
    const homeCategoryImages={arabic:'assets/4k/arabic.jpg',specialty:'assets/4k/specialty.jpg',tea:'assets/4k/tea.jpg',sparkling:'assets/4k/sparkling.jpg'};
    const cards=order.map((k,i)=>{const c=C[k],image=homeCategoryImages[k];return `<article class="categoryCard tiltCard" tabindex="0" role="button" data-go="/collection/${k}"><img src="${image}" alt="${esc(t(c.en,c.ar))}" loading="lazy"><span class="categoryMonogram" aria-hidden="true">MJ</span><div class="cardCopy"><div class="eyebrow">0${i+1}</div><h3>${esc(t(c.en,c.ar))}</h3><p>${esc(t(c.subEn,c.subAr))}</p><span class="enterArrow">↗</span></div></article>`}).join('');
